...
TEST_CASE("Adaptive Runge-Kutta 5th order circular flow test") {
   ...
   Read read;
   read.coords = {{-2.0, -1.0, 0.0, 1.0, 2.0}, {-2.0, -1.0, 0.0, 1.0, 2.0}, {0.0, 1.0}};
   read.values.resize(3);
   for (int i = 0; i < 5; i++) {
      for (int j = 0; j < 5; j++) {
         for (int k = 0; k < 2; k++) {
            read.values[0].push_back(-read.coords[1][j]);
            read.values[1].push_back(read.coords[0][i]);
            read.values[2].push_back(0.0);
         }
      }
   }
