...
struct Integrator {
   ...
   void integrate(std::array<std::pair<double, double>, 3> bounds, double y_wrap, bool geographic);
...
template <class Stepper>
void Integrator<Stepper>::integrate(std::array<std::pair<double, double>, 3> bounds, double y_wrap, bool geographic) {
   ...
   for (num_steps = 0; num_steps < MAXSTP; num_steps++) {
      ...
      if (geographic) { 
         if (pos[1] < bounds[1].first) pos[1] += y_wrap;
         else if (pos[1] > bounds[1].second) pos[1] -= y_wrap;
      }
